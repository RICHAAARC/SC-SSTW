...
def load_wan(c, guard, *, torch=None, pipeline_class=None, vae_class=None, record=None):
    ...
    if record: record(dict(model=model, scheduler_class=type(pipe.scheduler).__name__, scheduler_config=dict(pipe.scheduler.config), timesteps=pipe.scheduler.timesteps.detach().cpu().tolist(), latent_shape=list(initial.shape), latent_dtype=str(initial.dtype), transformer_dtype=str(next(pipe.transformer.parameters()).dtype), vae_dtype=str(next(vae.parameters()).dtype), offload=False, transformer_checkpointing=bool(getattr(pipe.transformer, 'is_gradient_checkpointing', False)), vae_checkpointing=bool(getattr(vae, 'is_gradient_checkpointing', False)), transformer_resolved_revision=getattr(pipe.transformer.config, '_commit_hash', None), vae_resolved_revision=getattr(vae.config, '_commit_hash', None)))
