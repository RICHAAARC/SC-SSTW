...
def _direction(key: bytes, bit: int, length: int, device: Any) -> Any:
    import torch
    seed=int.from_bytes(hmac.new(key,PROTOCOL_NAMESPACE+b"/direction/"+bit.to_bytes(2,"big"),hashlib.sha256).digest()[:8],"big")
    generator=torch.Generator(device=device).manual_seed(seed)
    v=torch.randn(length,generator=generator,device=device,dtype=torch.float32); return v/v.norm()
...
def write(normalized_latent: Any,key: bytes,message: bytes) -> tuple[Any,list[dict[str,float|int]]]:
    ...
    for index,(start,y,x,block) in enumerate(supports(out)):
        bit=bits[index%PAYLOAD_BITS]; direction=_direction(key,index%PAYLOAD_BITS,block.numel(),block.device).reshape_as(block); before=float((block.float()*direction).sum())
        target=MARGIN if bit else -MARGIN; delta=max(0.0,target-before) if bit else min(0.0,target-before)
        block.add_(direction.to(block.dtype)*delta); after=float((block.float()*direction).sum()); records.append({"support":index,"bit_index":index%PAYLOAD_BITS,"tubelet_start":start,"y":y,"x":x,"projection_before":before,"projection_after":after})
    return out,records
...
def read(normalized_latent: Any,key: bytes) -> dict[str,Any]:
    ...
    for index,(_,_,_,block) in enumerate(supports(normalized_latent)):
        votes[index%PAYLOAD_BITS].append(float((block.float()*_direction(key,index%PAYLOAD_BITS,block.numel(),block.device).reshape_as(block)).sum()))
