...
DEFAULT_OUTPUT = ROOT / "notebooks/rgb_dct_multistep_comparison_v1_colab.ipynb"


def _code(source: str, index: int) -> dict:
    return dict(cell_type="code", execution_count=None, metadata={}, outputs=[],
                source=source.splitlines(keepends=True), id=f"rgb-dct-multistep-{index}")
...
setup_slots = dict(
    ...
    fixed_denominator=dict(sources=2, mp4_score_slots=8, frames=1448),
    cases={case_id: {arm: dict(status='NOT_RUN_GLOBAL_SETUP_FAILURE', reason='SETUP_NOT_COMPLETED')
                     for arm in ('OFF', 'SINGLE49', 'SINGLE46', 'MULTI44_46')}
           for case_id in ('eval_kite_s2421', 'eval_pottery_s2422')},
)
    ...
    if check and returncode:
        ...
        for slots in setup_slots['cases'].values():
            for slot in slots.values():
                slot['reason'] = 'SETUP_COMMAND_EXIT_' + str(returncode)
        SETUP_SLOTS_PATH.write_text(json.dumps(setup_slots, indent=2) + '\\n', encoding='utf-8')
        raise subprocess.CalledProcessError(returncode, command)
    return subprocess.CompletedProcess(command, returncode)
"""
    install = _install_source(golden)
    checkout = """REPO = Path('/content/SC-SSTW-RGB-DCT-MULTISTEP-' + stamp)
logged_run(['git', 'clone', '--filter=blob:none', 'https://github.com/RICHAAARC/SC-SSTW.git', str(REPO)])
logged_run(['git', '-C', str(REPO), 'fetch', 'origin', 'dev/rgb-dct-multistep-comparison-v1'])
...
command = [sys.executable, '-u', '-m', 'experiments.wan_state_clock.rgb_dct_multistep_comparison_run', '--output', str(OUTPUT)]
...
"""
    cells = [
        ...
        dict(cell_type="markdown", metadata={}, id="rgb-dct-multistep-purpose", source=[
            "# RGB-DCT T44/T46/T49 fixed multistep comparison\n", "\n",
            "Run all once. Each of two fresh sources attempts OFF, SINGLE49, SINGLE46, and MULTI44_46 from the same noise and full UniPC history. Each full MP4 is blindly scored by the fixed C>=24 rule; every failed or missing slot remains in the eight-slot denominator. The T44/T46 native-response targets split the same total step-RMS budget. Paired MP4 quality is diagnostic. The user performs the GPU/Colab run; this notebook does not establish low FPR, population detection, a video-quality pass, or general multistep gain.\n",
        ]),
    ...
    notebook = dict(
        ...
        metadata=dict(
            accelerator="GPU",
            colab={"name": "RGB-DCT T44 T46 T49 fixed comparison", "provenance": []},
