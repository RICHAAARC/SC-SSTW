...
DEFAULT_OUTPUT = ROOT / "notebooks/rgb_dct_group_consistency_v1_colab.ipynb"


def _code(source: str, index: int) -> dict:
    return dict(cell_type="code", execution_count=None, metadata={}, outputs=[],
                source=source.splitlines(keepends=True), id=f"rgb-dct-group-{index}")
...
"""
    ...
    checkout = """REPO = Path('/content/SC-SSTW-RGB-DCT-GROUP-' + stamp)
logged_run(['git', 'clone', '--filter=blob:none', 'https://github.com/RICHAAARC/SC-SSTW.git', str(REPO)])
logged_run(['git', '-C', str(REPO), 'fetch', 'origin', 'dev/rgb-dct-group-consistency-h0-h1-v1'])
...
command = [sys.executable, '-u', '-m', 'experiments.wan_state_clock.rgb_dct_group_consistency_run', '--output', str(OUTPUT)]
...
"""
    cells = [
        ...
        dict(cell_type="markdown", metadata={}, id="rgb-dct-group-purpose", source=[
            "# RGB-DCT T49 group-consistency blind H0/H1 functional run\n", "\n",
            "Run all once. The fixed C>=24 rule is applied to each actual full MP4 readback. Two REF_OFF sources are independent negative witnesses and do not calibrate C. Two EVAL sources each attempt OFF and NATIVE_H1. Six slots and every failure remain in result.json. This tests functional presence on two sources only, not low FPR, population detection, video quality, payload, attribution, or multistep gain. The user performs the GPU/Colab run.\n",
        ]),
    ...
    notebook = dict(
        ...
        metadata=dict(
            accelerator="GPU",
            colab={"name": "RGB-DCT T49 group consistency H0 H1", "provenance": []},
