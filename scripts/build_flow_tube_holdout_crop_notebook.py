...
def build(source_commit=None):
    if source_commit is not None:
        ...
        subprocess.run(['git','-C',str(ROOT),'cat-file','-e',source_commit+':experiments/wan_state_clock/flow_tube_holdout_crop_run.py'],check=True)
    cells=[]
    ...
    cell('markdown','scope',"""# 原管状块接收器：固定跨内容RGB裁剪复现（准备版）

仅实现/CPU合成检查，本地未执行真实科研评分或GPU。Run all只加载VAE，不运行Transformer。
固定复用holdout run `flow_tube_state_holdout_20260920T033109891109Z` 的2case OFF/LAST_A/B共6源MP4，
验证原source SHA与文件hash。原4marked源各3个相关裁剪，非12个独立生成样本；两内容/种子曾用于反演验证，并非全项目未见。
每源真实RGB起点0/4/5、长度129，共18片段。start0也是129帧截短对照，不是完整181帧baseline。
裁剪存uint8.npy并实际回读；不再MP4压缩，避免额外codec混杂。
每片4内部origin真实RGB输入129/125/125/125，固定72VAEencode；0Transformer/0decode/0MP4save。
...
OFF只排名不FPR；每start固定4marked、2OFF，失败完整保留。间隔是二候选归因，不是payload比特恢复。

新输出 `MyDrive/Video-WM/FlowTubeHoldoutCrop/flow_tube_holdout_crop_<UTC>`；源run只读不覆写。
保存source路径/hash、裁剪raw range、npy/72观测、盲detection与完整逐行结果。
...
RUN_ID = 'flow_tube_holdout_crop_' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
...
""")
    cell('code','fixed-eighteen-crops',"""OUTPUT = Path('/content/drive/MyDrive/Video-WM/FlowTubeHoldoutCrop') / RUN_ID
...
command = [sys.executable, '-u', '-m', 'experiments.wan_state_clock.flow_tube_holdout_crop_run', '--output', str(OUTPUT)]
...
""")
    ...
    target=ROOT/'notebooks/flow_tube_holdout_crop_colab.ipynb';target.write_text(json.dumps(notebook,indent=1)+'\n');return target
