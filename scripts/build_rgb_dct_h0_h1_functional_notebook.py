...
DEFAULT_OUTPUT = ROOT / "notebooks/rgb_dct_h0_h1_functional_v1_colab.ipynb"


def _code(source: str, index: int) -> dict:
    return dict(cell_type="code", execution_count=None, metadata={}, outputs=[],
                source=source.splitlines(keepends=True), id=f"rgb-dct-h0h1-{index}")
...
"""
    ...
    checkout = """REPO = Path('/content/SC-SSTW-RGB-DCT-H0H1-' + stamp)
logged_run(['git', 'clone', '--filter=blob:none', 'https://github.com/RICHAAARC/SC-SSTW.git', str(REPO)])
logged_run(['git', '-C', str(REPO), 'fetch', 'origin', 'dev/rgb-dct-h0-h1-functional-v1'])
...
command = [sys.executable, '-u', '-m', 'experiments.wan_state_clock.rgb_dct_h0_h1_functional_run', '--output', str(OUTPUT)]
...
if result['fixed_denominator'] != {'sources': 4, 'mp4_score_slots': 6, 'frames': 1086}:
...
for case_id, case in result['cases'].items():
    for arm, row in case['slots'].items():
        print(case_id, arm, row['status'], row['score'], row['decision'], row['reason'], flush=True)
print('full result:', RESULT_PATH, flush=True)
"""
    cells = [
        _code("from google.colab import drive\ndrive.mount('/content/drive')", 0),
        dict(cell_type="markdown", metadata={}, id="rgb-dct-h0h1-purpose", source=[
            "# RGB-DCT T49 blind H0/H1 functional run\n", "\n",
            "Run all once. Two independent CAL_OFF MP4 scores freeze tau=max(scores)+1e-6 before either evaluation source starts. The two fixed EVAL sources each attempt OFF and NATIVE_H1; only actual MP4 readback scores enter the strict score>tau blind decisions. Six slots and every failure remain in result.json. This tests functional presence on two sources only, not low FPR, population detection, video quality, payload, attribution, or multistep gain. The user performs the GPU/Colab run.\n",
        ]),
    ...
    notebook = dict(
        ...
        metadata=dict(
            accelerator="GPU",
            colab={"name": "RGB-DCT T49 blind H0 H1 functional", "provenance": []},
