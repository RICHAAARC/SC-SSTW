"""Build the immutable-source Run-all notebook after source publication."""
...
DEFAULT_OUTPUT = ROOT / "notebooks/rgb_dct_local_gradient_v1_colab.ipynb"


def _code(source: str, index: int) -> dict:
    return dict(cell_type="code", execution_count=None, metadata={}, outputs=[],
                source=source.splitlines(keepends=True), id=f"rgb-dct-local-gradient-{index}")
...
def _failure_support_source() -> str:
    return """import subprocess, sys, json, traceback
...
def _guarded(stage: str, source: str) -> str:
    if not re.fullmatch(r"[A-Z_]+", stage):
        raise ValueError("uppercase setup stage required")
    return f"guarded_setup({stage!r}, {source!r})\n"
...
""".replace("SOURCE_SHA_TOKEN", repr(source_sha))
    failure_support = _failure_support_source()
    install = _install_source(golden)
    checkout = """REPO = Path('/content/SC-SSTW-RGB-DCT-LOCAL-GRADIENT-' + stamp)
...
"""
    environment = """import shutil, torch, numpy, diffusers
...
"""
    cells = [
        ...
        dict(cell_type="markdown", metadata={}, id="rgb-dct-local-gradient-purpose", source=[
        ...
        _code(setup, 1),
        _code(failure_support + _guarded("INSTALL", install), 2),
        _code(_guarded("SOURCE_CHECKOUT", checkout), 3),
        _code(_guarded("ENVIRONMENT", environment), 4),
        _code(_guarded("RUN", run), 5),
        _code(_guarded("RESULT_PRESENTATION", results), 6),
    ]
    notebook = dict(cells=cells, nbformat=4, nbformat_minor=5,
                    metadata=dict(accelerator="GPU",
                                  colab={"name": "RGB-DCT local gradient fixed trial",
                                         "provenance": []},
