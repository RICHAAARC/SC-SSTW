...
import json,math
import pytest,torch
from main.tube_state import grow_frequency as carrier,grow_control_budget as budget,grow_spatial_readout as scoring
from runtime.wan import grow_spatial_control as rt,grow_late_control as prior
from experiments.wan_state_clock import grow_spatial_control_run as gen,grow_spatial_control_full as full,grow_spatial_media as media
...
def test_native_multi_matches_prior_and_last_matches_energy(monkeypatch,native):
    ...
    rows=[];calls=[]
    off_args=(z,torch.tensor(1.),torch.tensor(-1.),torch.float32,5.,book,None)
    off_model=Model()
    off=rt.generate(SimpleNamespace(transformer=off_model,scheduler=native()),*off_args,lambda *x:None,lambda x:None,lambda *x:None,control_indices=rt.CONTROL_INDICES)
    old_off=prior.generate(SimpleNamespace(transformer=Model(),scheduler=native()),*off_args,lambda *x:None,lambda x:None,lambda *x:None,control_indices=prior.CONTROL_INDICES)
    torch.testing.assert_close(off,old_off,rtol=0,atol=0)
    assert not any(off_model.grad_enabled) and off_model.weight.grad is None
...
def test_hard_soft_and_candidate_attribution_distinct():
    agg={'votes_per_bit':184,'vote_sums':[2]*16,'signs':[1]*16,'coefficient_mean_diagnostic':[-100.]+[1.]*15}
...
def test_spatial_analytic_gradient_and_cfg_velocity_contract(monkeypatch):
    monkeypatch.setattr(carrier,'SHAPE',(1,2,46,12,12))
    z=torch.randn(carrier.SHAPE,dtype=torch.float64,generator=torch.Generator().manual_seed(3))*.1
    # Equal temporal neighbors remain a spatial signal; the difference carrier would annihilate them.
    z[:,:,1::2]=z[:,:,::2];book=carrier.codebook(b'spatial')
    leaf=z.clone().requires_grad_();g=torch.autograd.grad(carrier.loss(leaf,book,0),leaf)[0]
    torch.testing.assert_close(g,carrier.analytic_gradient(z,book,0),rtol=1e-11,atol=1e-11)
    assert torch.count_nonzero(g[:,1:])==0 and torch.count_nonzero(carrier.selected(z,book))>0
    spectrum=carrier.dct2(g[:,0:1]);mask=torch.ones((12,12),dtype=torch.bool);mask[2:10,2:10]=False
    assert spectrum[...,mask].abs().max()<1e-12
    after=z-carrier.ETA*g
    assert float(carrier.loss(after,book,0)/carrier.loss(z,book,0))==pytest.approx(.81,abs=1e-12)
    assert carrier.read(z,book)['aggregate']['votes_per_bit']==184
    sigma=.4;cond=z*.2+.1;uncond=z*.05-.07;v=uncond+5*(cond-uncond)
    u,controlled,clean_after,row=rt.pulse(z,v,sigma,book,0,lambda *a:None)
    expected=-.1*carrier.analytic_gradient(z-sigma*v,book,0)
    torch.testing.assert_close(u,expected,rtol=1e-11,atol=1e-11)
    torch.testing.assert_close(controlled,v-u/sigma,rtol=0,atol=0)
    torch.testing.assert_close(clean_after-(z-sigma*v),u,rtol=1e-10,atol=1e-12)
    conditional_update=-.1*carrier.analytic_gradient(z-sigma*cond,book,0)*5
    assert not torch.allclose(u,conditional_update)
    assert not u.requires_grad and not controlled.requires_grad


def test_spatial_failure_placeholders_and_no_temporal_transform():
    gen.validate(gen.load(gen.MANIFEST));assert carrier.CONTROL_INDICES==tuple(range(10,30))
    manifest=gen.load(gen.MANIFEST);assert 'temporal_carrier' not in manifest['frequency']
    for video in gen.empty('FAILED')['videos'].values():
        assert len(video['terminal']['per_time'])==46 and 'per_pair' not in video['terminal']
    assert len(media.empty_layer()['per_time'])==46


def test_partial_saved_mp4_and_nonfinite_readback_not_success(monkeypatch,native,tmp_path):
    monkeypatch.setattr(carrier,'SHAPE',(1,2,46,12,12));monkeypatch.setattr(media,'RGB_SHAPE',(3,4,4,3))
    z=torch.zeros(carrier.SHAPE);pipe=SimpleNamespace(transformer=Model(),scheduler=native())
    monkeypatch.setattr(gen,'prepare_generation',lambda *a,**kw:(pipe,z,torch.tensor(1.),torch.tensor(-1.),torch.float32))
    root=tmp_path/'generation';case=gen.CASES[0];g=gen.run_case(case,root/case)
    (root/'result.json').write_text(json.dumps({'cases':{case:g}}));vae=torch.nn.Linear(1,1);vae.config=SimpleNamespace(_commit_hash='fake')
    monkeypatch.setattr(media,'load_frozen_vae',lambda *a:vae);monkeypatch.setattr(media,'decode_normalized_latent',lambda *a:torch.full(media.RGB_SHAPE,.5));monkeypatch.setattr(media,'reencode_rgb24_readback',lambda *a:torch.zeros(carrier.SHAPE))
    reads=[]
    def save(rgb,path,*a):
        path.parent.mkdir(exist_ok=True);path.write_bytes(b'partial')
        if path.stem=='MULTI_A':raise RuntimeError('partial write')
    def read(path):
        reads.append(path.stem);return torch.full(media.RGB_SHAPE,float('nan') if path.stem=='LAST_A' else .5)
    monkeypatch.setattr(media,'encode_rgb',save);monkeypatch.setattr(media,'read_mp4',read)
    r=media.run_case(case,tmp_path/'media'/case,root,expected_source_run='generation',expected_source_commit=g['source_commit'],expected_manifest=gen.load(gen.MANIFEST),arms=gen.ARMS,messages=gen.MESSAGES,compare_readouts=True)
    assert 'MULTI_A' not in reads and r['status']=='WITH_RETAINED_FAILURES'
    assert r['videos']['MULTI_A']['layers']['mp4']['status']=='FAILED_SAVE_OR_READBACK'
    assert r['videos']['LAST_A']['layers']['mp4']['status']=='FAILED_SAVE_OR_READBACK'
    assert r['videos']['LAST_B']['layers']['mp4']['status']=='COMPLETE'
    assert len(r['videos'])==5 and all(len(v['layers'])==4 for v in r['videos'].values())
