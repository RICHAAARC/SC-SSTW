...
from . import grow_paired_control_run as generation
    ...
    for case in CASES:
        ...
        try:
            with log.open('w') as stream:
                child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.grow_paired_control_full','--output',str(output),'--media-case',case],stdout=stream,stderr=subprocess.STDOUT,check=False)
