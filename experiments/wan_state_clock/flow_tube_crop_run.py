"""Fixed real-RGB crop diagnostic from twelve saved MP4s. No Transformer or generation."""
...
MANIFEST=Path(__file__).parent/'configs/flow_tube_crop.json'
SOURCE_RUN='flow_tube_state_guidance_20260919T202545864571Z'
SOURCE_COMMIT='892013f44a5ab1596386f5e7046548ca4150d44e'
SOURCE_ROOT=Path('/content/drive/MyDrive/Video-WM/FlowTubeStateGuidance')/SOURCE_RUN
...
def validate_manifest(m):
    expected=dict(protocol='flow_tube_real_crop_v1',source_run=SOURCE_RUN,source_commit=SOURCE_COMMIT,cases=list(CASES),arms=list(ARMS),starts=list(STARTS),frames=FRAMES,origins=[0,1,2,3],storage='uint8_npy_no_second_codec',source_shape=list(SOURCE_SHAPE),crop_shape=list(CROP_SHAPE),no_search=analysis.NO_SEARCH)
...
def summarize(cases):
    ...
    for start in STARTS:
        ...
        for mode in MODES:
            ...
            result[str(start)][mode]=dict(marked_denominator=8,complete=done,missing_or_failed=8-done,unique_correct=correct,message_margins=gaps,coverage=coverage,window_mapping=mapping,OFF=off,claim='two-candidate attribution and window mapping, not FPR or bit payload recovery')
    return result
    ...
    for start,modes in summary.items():
        for mode,row in modes.items():
            ...
            rows.append(dict(start=int(start),mode=mode,denominator=row['marked_denominator'],complete=row['complete'],missing_or_failed=row['missing_or_failed'],unique_correct=row['unique_correct'],
                ...
                OFF_denominator=4,OFF_complete=sum(v['status']=='COMPLETE' for v in row['OFF'])))
    return rows


def run_all(output,source_root=SOURCE_ROOT):
    output,source_root=validate_output(output,source_root);output.mkdir(parents=True,exist_ok=False)
    result=dict(status='RUNNING',source_root=str(source_root),source_video_denominator=12,fragment_denominator=36,marked_denominator=24,receiver_encode_denominator=144,cases={c:empty('NOT_RUN') for c in CASES},fixed_calls={k:4*v for k,v in PLAN.items()},scientific_pass=None)
    ...
    for case in CASES:
        ...
        try:
            with log.open('w') as f:child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.flow_tube_crop_run','--output',str(output/case),'--case-id',case,'--source-root',str(source_root)],stdout=f,stderr=subprocess.STDOUT,check=False)
