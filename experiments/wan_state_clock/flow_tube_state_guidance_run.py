"""Fixed original tube/state code and receiver: terminal versus native last-step bridge."""
...
ARMS=('OFF','TERMINAL_A','TERMINAL_B','LAST_A','LAST_B')
...
PLAN={'transformer':100,'scheduler_step':52,'vae_decode':5,'mp4_save':5,'vae_encode':20}
MANIFEST=Path(__file__).parent/'configs/flow_tube_state_guidance.json'
...
def validate(manifest):
    ...
    config=copy.deepcopy(base['base_config']);config['generation']['role']='shared_prefix49_terminal_vs_last_full_projection'
    config['output_drive_parent']='/content/drive/MyDrive/Video-WM/FlowTubeStateGuidance'
    expected=dict(protocol='flow_tube_state_guidance_v1',base_config=config,development=base['development'],arms=list(ARMS),control_index=49,margin=1.,numeric_atol=2e-5,numeric_rtol=2e-4,receiver_origins=[0,1,2,3],quality_tolerance=None)
...
def generate_case(case_id,output):
    ...
    try:
        ...
        for m,suffix in enumerate(('A','B')):
            try:
                terminal_np,evidence=carrier.write(off.cpu().numpy(),book,m);terminal=torch.from_numpy(terminal_np)
                store('TERMINAL_'+suffix,terminal);dump(output/'write_evidence'/('TERMINAL_'+suffix+'.json'),evidence)
            except Exception as exc:result['videos']['TERMINAL_'+suffix]['status']='FAILED';fail('TERMINAL_'+suffix,exc)
            try:
                ...
                if (output/('TERMINAL_'+suffix+'_terminal.pt')).exists():
                    direct=torch.load(output/('TERMINAL_'+suffix+'_terminal.pt'),map_location='cpu',weights_only=True)
                    result['equivalence']['LAST_'+suffix+'_vs_TERMINAL']=method.numerical_equivalence(last.cpu().numpy(),direct.numpy())
...
def summary(cases):
    ...
    for group in ('TERMINAL','LAST'):
        ...
        for mode in MODES:
            ...
            out[group][mode]=dict(marked_denominator=8,complete=done,missing_or_failed=8-done,unique_correct=correct)
    return out


def run_all(output):
    output=Path(output);output.mkdir(parents=True,exist_ok=False);manifest=load(MANIFEST);validate(manifest)
    result=dict(status='RUNNING',video_denominator=20,receiver_encode_denominator=80,cases={c:empty('NOT_RUN') for c in CASES},fixed_calls={k:4*v for k,v in PLAN.items()},scientific_pass=None,quality_tolerance=None)
    ...
    for stage in ('generate','media'):
        for case in CASES:
            ...
            try:
                with log.open('w') as f:child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.flow_tube_state_guidance_run','--output',str(output/case),'--case-id',case,'--stage',stage],stdout=f,stderr=subprocess.STDOUT,check=False)
