"""Spatial-carrier fixed five-arm four-layer diagnostic. Model execution only through CLI."""
...
def empty_layer():return {'status':'NOT_RUN','latent_time_denominator':46,'per_time':[{'time':k,'status':'NOT_RUN'} for k in range(46)]}
...
def run_case(case_id,output,source_root,*,expected_source_run=SOURCE_RUN,expected_source_commit=SOURCE_COMMIT,expected_manifest=None,arms=ARMS,messages=None,compare_readouts=False):
    ARMS=tuple(arms)
    message_for={a:(None if a=='OFF' else 0 if a.endswith('A') else 1) for a in ARMS} if messages is None else messages
    ...
    def read_layer(arm,layer,z):
        ...
        try:
            ...
            if compare_readouts:
                from main.tube_state.grow_spatial_readout import score,compare,candidate_scores
    ...
    try:
        ...
        result['media_source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),MANIFEST,Path(method.__file__),Path(sys.modules[decode_normalized_latent.__module__].__file__),Path(sys.modules[load_frozen_vae.__module__].__file__)]}
        import diffusers
        ...
        for arm in ARMS:
            ...
            try:
                ...
                try:
                    ...
                    if images['mp4'].shape!=rgb.shape or not torch.isfinite(images['mp4']).all():raise ValueError('MP4 readback geometry/finite differs')
