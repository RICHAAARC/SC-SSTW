...
from runtime.wan.grow_paired_control import generate as base_generate,CONTROL_INDICES,observation
...
MANIFEST=Path(__file__).parent/'configs/grow_paired_control.json'
...
def validate(manifest):
    ...
    expected=copy.deepcopy(old);expected['protocol']='grow_paired_control_v1'
    expected['matching']='paired_sum_native_response_rms_squared'
    expected['arms']=list(ARMS);expected['messages']=MESSAGES
    if manifest!=expected or method.ETA!=.1 or method.AMPLITUDE!=.5 or method.CONTROL_INDICES!=tuple(range(30,50)):raise ValueError('fixed paired protocol mismatch')
...
def run_case(case_id,output):
    ...
    try:
        ...
        result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in
            [Path(__file__),MANIFEST,Path(method.__file__),Path(method.spatial.__file__),Path(budget.__file__),Path(sys.modules[base_generate.__module__].__file__),Path(sys.modules[generate.__module__].__file__),
    ...
    for case in CASES:
        ...
        try:
            with log.open('w') as stream:
                child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.grow_paired_control_run',
