"""Fixed two-source OFF/SINGLE49/TERMINAL46 RGB-DCT experiment."""
...
CONFIG_PATH = Path(__file__).parent / "configs/rgb_dct_terminal_gradient_v1.json"
PROTOCOL_PATH = ROOT / "docs/rgb_dct_terminal_gradient_v1_protocol.md"
MODULE = "experiments.wan_state_clock.rgb_dct_terminal_gradient_run"
...
PLAN = dict(
    generation=2,
    transformer_prefix=200, scheduler_prefix=100,
    transformer_forecast=12, scheduler_forecast=8,
    vae_gradient_decode=2, vae_decode=4, vae_encode=4, vae_vjp=2,
    transformer_replay=12, scheduler_replay=8, tail_vjp=2,
    unit_response_probe_step=4, scheduler_response_shadow=2,
    scheduler_step=4, transformer_control_tail=12, scheduler_control_tail=6,
    mp4_save=6, mp4_read=6, score=6,
)
DERIVED = dict(transformer=236, scheduler_and_probe=132,
               vae_decode=6, backward_vjp=4)
...
def load_config() -> dict:
    ...
    if _sha(PROTOCOL_PATH) != config["protocol_sha256"]:
        raise ValueError("terminal-gradient protocol byte identity mismatch")
    if config["frozen_task_card_sha256"] != "d1ab995b1a1525e1996e1df9d670afe009a5a8a40a52f51e813ab2fbbdaff071":
        raise ValueError("frozen route-B task card identity changed")
    ...
    if config["control"] != dict(
        gradient_index=46, reference_index=49, latent_support_time=[1, 45],
...
def initial_result(config: dict, output: Path, source_sha: str) -> dict:
    ...
    for source in config["cases"]:
        ...
        cases[case_id] = dict(
            ...
            generation=None, terminal_forecast=None, proxy=None, gradient=None,
    ...
    return dict(
        ...
        call_completion_definition={
            ...
            "tail_vjp": "one attached native T46-through-T49 cotangent-to-v46 VJP",
        },
...
def run_case(store: Store, case_id: str, config: dict, backend,
    ...
    try:
        ...
        case["stage"] = "NO_GRADIENT_T46_TERMINAL_FORECAST"
        store.save()
        case["terminal_forecast"] = backend.forecast_terminal46()
        store.save()

        case["stage"] = "FP32_VAE_TERMINAL_COTANGENT"
        store.save()
        ...
        try:
            ...
            _fail_arm(store, case_id, "TERMINAL46", case["stage"], exc)
            _dependency_invalid(
                store, case_id, "SINGLE49",
                f"DEPENDENCY_SHARED_OFF_COTANGENT_FAILED:{type(exc).__name__}:{exc}",
            )
        ...
        if not any(case["slots"][arm]["status"] == "PENDING"
                   for arm in ("SINGLE49", "TERMINAL46")):
            return

        case["stage"] = "TRANSFORMER_RESTORE"
        store.save()
        case["phase_receipts"].append(backend.to_transformer_phase("TAIL_VJP_RESUME"))
        store.save()

        if case["slots"]["TERMINAL46"]["status"] == "PENDING":
            try:
                case["stage"] = "FULL_T46_T49_TERMINAL_VJP"
                store.save()
                case["gradient"] = backend.terminal_vjp()
                store.save()
                case["stage"] = "TERMINAL46_NATIVE_RESPONSE_AND_FREE_TAIL"
                store.save()
                state, _, control = backend.control_terminal46(R_STAR)
                case["controls"]["TERMINAL46"] = control
                store.save()
                if state != "READY":
                    _no_media_result(store, case_id, "TERMINAL46", state)
            except Exception as exc:
                _fail_arm(store, case_id, "TERMINAL46", case["stage"], exc)
        ...
        if case["stage"] in {
            "SHARED_OFF_TRAJECTORY", "NO_GRADIENT_T46_TERMINAL_FORECAST",
            "FP32_VAE_TERMINAL_COTANGENT",
        }:
            _dependency_invalid(
                store, case_id, "SINGLE49",
...
def finalize_experiment(store: Store) -> None:
    ...
    store.data["status"] = (
        "FIXED_TERMINAL_GRADIENT_COMPLETE" if complete and not mismatches
        else "FIXED_TERMINAL_GRADIENT_NEGATIVE" if complete else "INCOMPLETE"
    )
    store.save()
...
def main() -> None:
    ...
    config = load_config()
    output = args.output
    internal = os.environ.get("RGB_DCT_TERMINAL_GRADIENT_INTERNAL_CASE")
    if internal:
        ...
        backend = WanTerminalGradientBackend(
