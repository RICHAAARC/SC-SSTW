...
from runtime.wan.grow_temporal_difference import generate,CONTROL_INDICES,observation
...
def validate(manifest):
    old=load(MANIFEST.with_name('grow_late_control.json'))
    expected=copy.deepcopy(old);expected['protocol']='grow_temporal_difference_terminal_v1'
    expected['frequency']['temporal_carrier']={'pairs':[[2*k,2*k+1] for k in range(23)],'transform':'(even-odd)/sqrt(2)','votes_per_bit':92}
    if manifest!=expected or method.CONTROL_INDICES!=tuple(range(30,50)) or method.ETA!=.1 or method.AMPLITUDE!=.5:
...
def run_case(case_id,output):
    ...
    try:
        ...
        result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in
            [Path(__file__),MANIFEST,Path(method.__file__),Path(method.spatial.__file__),Path(sys.modules[generate.__module__].__file__),
    ...
    for case in CASES:
        ...
        try:
            with log.open('w') as stream:
                child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.grow_temporal_difference_run',
