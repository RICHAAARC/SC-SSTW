...
from runtime.wan.grow_temporal_difference import generate as base_generate,CONTROL_INDICES,observation
from main.tube_state import grow_signed_margin as margin
...
MANIFEST=Path(__file__).parent/'configs/grow_temporal_margin.json'
...
def generate(*args,**kwargs):
    return base_generate(*args,**kwargs,loss_fn=margin.loss)

def validate(manifest):
    old=load(MANIFEST.with_name('grow_temporal_difference.json'))
    expected=copy.deepcopy(old);expected['protocol']='grow_temporal_margin_terminal_v1'
    expected['writer_objective']={'type':'one_sided_squared_signed_margin','margin':.5}
    if manifest!=expected or method.ETA!=.1 or method.AMPLITUDE!=.5 or margin.MARGIN!=.5:raise ValueError('fixed margin candidate mismatch')
...
def run_case(case_id,output):
    ...
    try:
        ...
        result['source_sha256']={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in
            [Path(__file__),MANIFEST,Path(method.__file__),Path(method.spatial.__file__),Path(margin.__file__),Path(sys.modules[base_generate.__module__].__file__),Path(sys.modules[generate.__module__].__file__),
        ...
        for arm in ARMS:
            ...
            try:
                ...
                item['difference_domain_rms']=float(method.selected(z,book).double().square().mean().sqrt())
                item['terminal_candidate_losses']=[float(method.loss(z,book,m)) for m in (0,1)]
                item['terminal_writer_objective_losses']=[float(margin.loss(z,book,m)) for m in (0,1)]
...
def run_all(output):
    ...
    for case in CASES:
        ...
        try:
            with log.open('w') as stream:
                child=subprocess.run([sys.executable,'-u','-m','experiments.wan_state_clock.grow_temporal_margin_run',
    ...
    result['terminal_recovery_summary']=media_gate(result['cases'])
    result['terminal_recovery_summary'].pop('media_eligible',None)
    result['terminal_recovery_summary'].update(does_not_control_media=True)
