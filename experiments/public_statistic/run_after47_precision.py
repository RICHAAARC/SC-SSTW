"""One supervised saved-state precision diagnostic. Import performs no GPU/model work."""
import argparse
import json
import os
...
BUDGET=dict(transformer_calls=36,vae_calls=9,backward_calls=3,mp4=0,saved_frames=0,wall_seconds=1800)
REPLAY=dict(transformer_block=360,vae_chunk=39)
...
def worker(config,reference,output):
    ...
    try:
        ...
        diagnose_precision(adapter,snapshot,report,persist,save=lambda label,g:torch.save(g,out/(label+'.pt')))
...
def main():
    ...
    try:
        ...
        else:sys.exit(supervise(a,module="experiments.public_statistic.run_after47_precision",budget=BUDGET,replay=REPLAY))
