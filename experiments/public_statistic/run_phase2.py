...
def supervise(config_path, output):
    ...
    try:
        with (out/'execution.log').open('w') as log:
            ...
            while child.poll() is None:
                ...
                try:
                    ...
                    if rss > c['budget']['host_rss_gib']*2**30: reason = 'HOST_RSS_BUDGET'; break
                except (PermissionError, OSError, ValueError): reason = 'HOST_RSS_MONITOR_UNAVAILABLE'; break
                time.sleep(.2)
        ...
        write(out/'execution_exit.json', dict(command=command, returncode=child.returncode if child else None, stop=reason, elapsed_seconds=time.monotonic()-start, automatic_retries=0))
