...
def main():
    ...
    a=p.parse_args()
    if a.worker:
        from runtime.stage2.off_baseline import run
    ...
    cmd=[sys.executable,'-m','experiments.stage2.run_off_baseline','--worker','--config',str(Path(a.config).resolve()),'--output',str(out.resolve())]
    timed_out=False
