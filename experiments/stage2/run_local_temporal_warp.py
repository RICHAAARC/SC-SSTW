...
def main():
    ...
    if log.exists() or exit_path.exists():raise FileExistsError('retain prior execution records')
    cmd=[sys.executable,'-m','experiments.stage2.run_local_temporal_warp','--worker','--config',str(Path(a.config).resolve()),'--output',str(out.resolve())]
    timed_out=False
    with log.open('x') as stream:
        proc=subprocess.Popen(cmd,stdout=stream,stderr=subprocess.STDOUT,start_new_session=True)
        try:code=proc.wait(timeout=config['budgets']['wall_seconds'])
    ...
    record=dict(command=cmd,exit_code=code,timed_out=timed_out,elapsed_seconds=time.monotonic()-start,
                fixed_arms=list(config['outputs']),log=str(log),automatic_retries=0)
    exit_path.write_text(json.dumps(record,indent=2)+'\n')
    if timed_out:
        states={k:dict(status='NOT_COMPLETED_TIMEOUT') for k in config['outputs']}
        if (out/'output_status.json').exists():states.update(json.loads((out/'output_status.json').read_text()))
        record['prior_arm_status']=states.copy()
        states={k:dict(status='NOT_COMPLETED_TIMEOUT',prior=v) if v.get('status')=='RUNNING' else v for k,v in states.items()}
